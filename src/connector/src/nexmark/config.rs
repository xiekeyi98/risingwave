...
use super::{NEXMARK_BASE_TIME, NEXMARK_MAX_FETCH_MESSAGES};
...
pub struct NexmarkConfig {
    ...
    /// Maximum number of events generated for one call of Next, default value:
...
impl NexmarkConfig {
    pub fn from(properties: &Properties) -> anyhow::Result<Self> {
        ...
        let max_chunk_size = NexmarkConfig::get_as_or(
            properties,
            NEXMARK_CONFIG_MAX_CHUNK_SIZE,
            NEXMARK_MAX_FETCH_MESSAGES,
        )?;
        Ok(NexmarkConfig {
            active_people,
...
mod tests {
    ...
    fn test_config() -> Result<()> {
        let properties = Properties::new(
            hashmap! {NEXMARK_CONFIG_MAX_CHUNK_SIZE.to_string() => "512".to_string(),},
        );
